...
func SeedData() error {
    ...
    var count int
    if err := config.Pool.QueryRow(ctxBackground(), "SELECT COUNT(1) FROM movies").Scan(&count); err != nil {
        return err
    }
    if count > 0 {
        return nil
    ...
    type row struct{ title string; year int; genres string }
    ...
    flush := func(rows []row) error {
        ...
        _, err := config.Pool.Exec(ctxBackground(), sb.String(), args...)
        return err
    }
    ...
    for {
        ...
        if len(batch) >= batchSize {
            if err := flush(batch); err != nil { return err }
            batch = batch[:0]
        }
    }
    if err := flush(batch); err != nil { return err }
